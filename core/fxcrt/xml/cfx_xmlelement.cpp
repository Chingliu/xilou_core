...
CFX_XMLElement* CFX_XMLElement::GetFirstChildNamed_NoPrefix(WideStringView name) const {
  return GetNthChildNamed_NoPrefix(name, 0);
}

CFX_XMLElement* CFX_XMLElement::GetNthChildNamed_NoPrefix(WideStringView name,
                                                 size_t idx) const {
  for (auto* child = GetFirstChild(); child; child = child->GetNextSibling()) {
    CFX_XMLElement* elem = ToXMLElement(child);
    if (!elem || elem->GetLocalTagName() != name)
      continue;
    if (idx == 0)
      return elem;
...
CFX_XMLElement* CFX_XMLElement::GetNextSiblingNamed_NoPrefix(
    WideStringView name) {
  auto sibling = this->GetNextSibling();
  while (sibling) {
    CFX_XMLElement* elem = ToXMLElement(sibling);
    if (elem && elem->GetLocalTagName() == name)
      return elem;
