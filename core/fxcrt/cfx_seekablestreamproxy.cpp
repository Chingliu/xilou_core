...
std::pair<size_t, size_t> UTF8Decode(pdfium::span<const uint8_t> pSrc,
  ...
  if (iPending)
    iBackspace = iPending - iPendNum - 1;

  /*// debug log 
  WideString wbuf = WideString::Format(L"[UTF8Decode]--(%d)(%d)(backspace=%d)\n", iPendNum, iPending, iBackspace);
...
size_t CFX_SeekableStreamProxy::ReadBlock(wchar_t* pStr, size_t size) {
  ...
  size_t iSrc;
  int32_t iBackspace = 0;
  std::tie(iSrc, size) = UTF8Decode({buf.data(), iLen}, {pStr, size}, iBackspace);

  /*// debug log
  WideString wbuf = WideString::Format(L"[CFX_SeekableStreamProxy::ReadBlock]--(%s)(%d)\n", pStr, size);
  OutputDebugString(wbuf.c_str());
  //*/

  if (iBackspace < 0)
    Seek(From::Begin, m_iPosition + iBackspace);
  else
