...
void CPDF_PageContentGenerator::ProcessText(std::ostringstream* buf,
  ...
  *buf << static_cast<int>(pTextObj->GetTextRenderMode()) << " Tr ";
  ByteString text;
  //或许应该直接在PDF_TextObj中标识一下
  auto usingPdfRender = pFont->GetFontDict()->KeyExist("ToUnicode");
  if (usingPdfRender) {
    ...
    auto pofdTextObj = reinterpret_cast<COFD_TextObject*>(pTextObj);
    pFont->AppendChar(&text, pofdTextObj->m_GlyphIndex);
  }
