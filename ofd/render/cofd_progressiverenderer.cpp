...
using namespace ofd;
COFD_ProgressiveRenderer::COFD_ProgressiveRenderer(
...
void COFD_ProgressiveRenderer::Start(PauseIndicatorIface* pPause) {
...
void COFD_ProgressiveRenderer::Continue(PauseIndicatorIface* pPause) {
  while (m_Status == kToBeContinued) {
    if (!m_pCurrentLayer) {
      ...
      m_pRenderStatus = std::make_unique<COFD_RenderStatus>(m_pContext.Get(),
                                                            m_pDevice.Get());
      if (m_pOptions)
