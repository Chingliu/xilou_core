...
COFD_ImageRenderer::COFD_ImageRenderer() = default;

COFD_ImageRenderer::~COFD_ImageRenderer() = default;

bool COFD_ImageRenderer::StartLoadDIBBase() {
...
bool COFD_ImageRenderer::StartRenderDIBBase() {
...
bool COFD_ImageRenderer::Start(COFD_RenderStatus* pStatus,
...
const CPDF_RenderOptions& COFD_ImageRenderer::GetRenderOptions() const {
  return m_pRenderStatus->GetRenderOptions();
}

bool COFD_ImageRenderer::DrawPatternImage() {
  if (NotDrawing()) {
    m_Result = false;
    return false;
  }
...
bool COFD_ImageRenderer::DrawMaskedImage() {
  if (NotDrawing()) {
    m_Result = false;
    return false;
  }
...
bool COFD_ImageRenderer::StartDIBBase() {
...
bool COFD_ImageRenderer::StartBitmapAlpha() {
...
bool COFD_ImageRenderer::Continue(PauseIndicatorIface* pPause) {
  switch (m_Mode) {
    case Mode::kNone:
      return false;
    case Mode::kDefault:
...
bool COFD_ImageRenderer::ContinueDefault(PauseIndicatorIface* pPause) {
...
bool COFD_ImageRenderer::ContinueBlend(PauseIndicatorIface* pPause) {
  return m_pRenderStatus->GetRenderDevice()->ContinueDIBits(
      m_DeviceHandle.get(), pPause);
}

bool COFD_ImageRenderer::ContinueTransform(PauseIndicatorIface* pPause) {
...
void COFD_ImageRenderer::HandleFilters() {
...
absl::optional<FX_RECT> COFD_ImageRenderer::GetUnitRect() const {
...
bool COFD_ImageRenderer::GetDimensionsFromUnitRect(const FX_RECT& rect,
                                                   int* left,
                                                   int* top,
                                                   int* width,
                                                   int* height) const {
...
//static
void COFD_ImageRenderer::WriteBmp(const char* pdf_name,
                     int num,
                     CFX_DIBBase *pDIB) {
#ifdef _WIN32
  if (!pDIB)
    return;
  void* buffer = pDIB->GetBuffer();
  int stride = pDIB->GetPitch();
